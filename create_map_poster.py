...
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend — faster, no GUI overhead
import matplotlib.colors as mcolors
import matplotlib.pyplot as plt
import numpy as np
import osmnx as ox
from geopandas import GeoDataFrame
# geopy not available on PyPI currently, skip geocoding feature
...
def get_edge_widths_by_type(g, multiplier=1.0):
    """
    Assigns line widths to edges based on road type.
    Major roads get thicker lines. Multiplier scales all widths.
    """
    edge_widths = []
    ...
    for _u, _v, data in g.edges(data=True):
        ...
        edge_widths.append(width * multiplier)
...
def create_poster(
    ...
    """
    # Default layers: roads, water, parks
    if layers is None:
        layers = {"roads", "water", "parks"}
    else:
        layers = set(layers)

    # Custom label overrides display_city
    if custom_label:
        display_city = custom_label

    # Handle display names for i18n support
    # Priority: custom_label > display_city/display_country > name_label/country_label > city/country
    ...
    with tqdm(
        ...
        # 1. Fetch Street Network (must be first — we need it to render)
        ...
        # 2 & 3. Fetch features in parallel (only those enabled by layers)
        from concurrent.futures import ThreadPoolExecutor
        feature_layers = []
        if "water" in layers:
            feature_layers.append(("water", {"natural": "water", "waterway": "riverbank"}, compensated_dist))
        if "parks" in layers:
            feature_layers.append(("parks", {"leisure": "park", "landuse": "grass"}, compensated_dist))
        if "buildings" in layers:
            # Cap buildings fetch radius to 5 km to avoid massive OSM downloads
            bld_dist = min(compensated_dist, 5000)
            feature_layers.append(("buildings", {"building": True}, bld_dist))
        if "railways" in layers:
            feature_layers.append(("railways", {"railway": True}, compensated_dist))

        pbar.set_description("Downloading features")
        features = {}
        if feature_layers:
            with ThreadPoolExecutor(max_workers=max(len(feature_layers), 1)) as pool:
                futures = {
                    name: pool.submit(fetch_features, point, feat_dist, tags, name)
                    for name, tags, feat_dist in feature_layers
                }
                for name, fut in futures.items():
                    try:
                        features[name] = fut.result()
                    except Exception as e:
    ...
    parks = features.get("parks")
    buildings = features.get("buildings")
    railways = features.get("railways")
    ...
    if "water" in layers and water is not None and not water.empty:
    ...
    if "buildings" in layers and buildings is not None and not buildings.empty:
        bld_polys = buildings[buildings.geometry.type.isin(["Polygon", "MultiPolygon"])]
        if not bld_polys.empty:
            try:
                bld_polys = ox.projection.project_gdf(bld_polys)
            except Exception:
                bld_polys = bld_polys.to_crs(g_proj.graph['crs'])
            bld_color = THEME.get('buildings', THEME.get('road_default', '#555555'))
            bld_polys.plot(ax=ax, facecolor=bld_color, edgecolor='none', alpha=0.5, zorder=0.6)

    if "parks" in layers and parks is not None and not parks.empty:
        # Filter to only polygon/multipolygon geometries to avoid point features showing as dots
    ...
    if "railways" in layers and railways is not None and not railways.empty:
        # Railways can be LineString or MultiLineString
        ...
        if not rail_lines.empty:
            ...
            rail_color = THEME.get('railway', THEME.get('road_default', '#777777'))
            rail_lines.plot(ax=ax, color=rail_color, linewidth=0.8, alpha=0.7, zorder=0.9, linestyle='--')

    # Layer 2: Roads with hierarchy coloring
    if "roads" in layers:
        print("Applying road hierarchy colors...")
        edge_colors = get_edge_colors_by_type(g_proj)
        edge_widths = get_edge_widths_by_type(g_proj, multiplier=road_thickness)
    ...
    # Attribution text removed per user request
    # ax.text(
    #     0.98,
    #     0.02,
...
Examples:
  ...
  python create_map_poster.py --list-themes
    ...
    parser.add_argument(
        ...
        type=int,
        default=150,
        help="Image DPI / quality (default: 150). Common values: 72 (web), 150 (standard), 300 (print)",
    )
    parser.add_argument(
        "--layers",
        type=str,
        default="roads,water,parks",
        help="Comma-separated list of layers to render (default: roads,water,parks). Options: roads,water,parks,buildings,railways",
    )
    parser.add_argument(
        "--road-thickness",
        dest="road_thickness",
        type=float,
        default=1.0,
        help="Road thickness multiplier (default: 1.0). Range: 0.5 to 3.0",
    )
    parser.add_argument(
        "--custom-label",
        dest="custom_label",
        type=str,
        help="Custom label text to replace the city name on the poster",
    )
    parser.add_argument(
        "--format",
        "-f",
